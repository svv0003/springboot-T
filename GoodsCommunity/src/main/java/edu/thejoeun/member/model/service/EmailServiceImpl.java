...
public class EmailServiceImpl implements EmailService {
    ...
    @Autowired
    private JavaMailSender mailSender;              // EmailConfig 에  설정된 메일보내기 기능 과 관련 환경설정 사용
    @Autowired
    private EmailMapper emailMapper;
    @Autowired
    private SpringTemplateEngine templateEngine;    // 템플릿 엔진 이용해서 auth/signup.html 에 있는 html 코드를 java로 변환

    // 이메일 보내기
    @Override
    public boolean sendMail(String htmlName, String email) {
        ...
        try{
            ...
            emailMapper.insertAuthKey(Map.of("email", email, "authKey", authKey));
            return true;
        } catch (Exception e) {
            log.error("인증메일 발송 실패 - email: {}", email, e);
            return false;
        }
    }
        /*
    ...
    public int checkAuthKey(Map<String, Object> map) {
        ...
        Email storedKey = emailMapper.getAuthKeyByEmail(email);

        log.info("inputKey : {}, storedKey : {}", inputKey, storedKey);

        if (storedKey == null || storedKey.equals(inputKey)) {
            log.info("email: {} 에 해당하는 인증키가 DB에 없습니다.", email);
            return 0; // 인증키 없음
        }
        return 0;
    }
